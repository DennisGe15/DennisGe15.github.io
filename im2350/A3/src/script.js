...
var Delighters = new (function() {
	...
	// The purpose of this code is to trigger init if autoInit is true after the content of the document has been loaded.

	function config(opts) {
		for (var name in opts) options[name] = opts[name];
	}
	// This code is used to configure the options.
	...
	function init() {		
		...
		for (var i=0; i<els.length; i++) {
		    ...
		    // This is the wire frame size.

		scroll();
	}

	function scroll() {
	// Loop to get Windows of different sizes.
		...
		for (var i=0; i<dels.length; i++) {
			...
			if (del.debug) {
				if (factorStart >= 0 && factorStart <= 1) {
					...
					// The element is positioned relative to the viewport, which means that it is always in the same place even if the page is scrolling.
...
var Delighters = new function() {
    var t = this.dels = [],
        e = {
            attribute: "data-delighter",
            classNames: ["delighter", "started", "ended"],
			// This is class attribute definition.
...
// The main purpose of this function is to process the elements in the document with the specified attributes when the scrolling effect occurs, parsing the values of the attributes and converting them into configuration objects,
// and then triggering different animation effects based on these configuration objects. Here, I've given a brief definition of some of the longer functions.
